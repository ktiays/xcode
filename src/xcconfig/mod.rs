...
pub fn build(config: &XCConfig) -> String {
    ...
    for include in &config.includes {
        let optional = if include.include.optional { "?" } else { "" };
        lines.push(format!("#include{} \"{}\"", optional, include.include.path));
    }

    if !config.includes.is_empty() && !config.build_settings.is_empty() {
        lines.push(String::new());
    }

    for setting in &config.build_settings {
        let mut key = setting.key.clone();
        if let Some(conditions) = &setting.conditions {
            for condition in conditions {
                if let Some(sdk) = &condition.sdk {
                    key.push_str(&format!("[sdk={}]", sdk));
                }
                if let Some(arch) = &condition.arch {
                    key.push_str(&format!("[arch={}]", arch));
                }
                if let Some(config) = &condition.config {
                    key.push_str(&format!("[config={}]", config));
                }
            }
        }
        lines.push(format!("{} = {}", key, setting.value));
    }
